...
RUN apt-get update && \
  ...
  libtiff5 libvpx3 libxpm4 libxslt1.1 libzip4 php php-curl php-gd php-imagick php-ldap php-xml php-zip php7.2-curl php7.2-gd php7.2-ldap php7.2-xml \
  php7.2-zip poppler-data ttf-dejavu-core apache2 libapache2-mod-php7.2 wget ca-certificates  && \
  wget http://prdownloads.sourceforge.net/lam/ldap-account-manager_${LAM_VERSION}-1_all.deb?download -O /tmp/ldap-account-manager_${LAM_VERSION}-1_all.deb && \
  dpkg -i /tmp/ldap-account-manager_${LAM_VERSION}-1_all.deb && apt-get purge wget ca-certificates -y  && apt-get autoremove -y && apt-get clean && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*
